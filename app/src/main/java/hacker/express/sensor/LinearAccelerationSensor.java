...
import hacker.express.filters.MeanFilter;
import hacker.express.sensor.observer.AccelerationSensorObserver;
import hacker.express.sensor.observer.GravitySensorObserver;
...
import hacker.express.sensor.observer.MagneticSensorObserver;
...
public class LinearAccelerationSensor implements GyroscopeSensorObserver,
    ...
    // Raw accelerometer data
    private float[] magnetic = new float[]{0, 0, 0};
    ...
    public void onGravitySensorChanged(float[] gravity, long timeStamp) {
        ...
        if (gravitySampleCount > MIN_SAMPLE_COUNT && magneticSampleCount > MIN_SAMPLE_COUNT
    ...
    private void notifyLinearAccelerationObserver() {
        for (LinearAccelerationSensorObserver a : observersAcceleration) {
            a.onLinearAccelerationSensorChanged(this.linearAcceleration, this.gyroscopeOrientation, this.timestampOld);
        }
    }
